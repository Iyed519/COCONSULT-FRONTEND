...
export class LiabilityFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.liabilityService.findByIdLiability({ id: +idParam }).subscribe({
      next: (data: Liability) => {
        // Si la réponse contient 'id' au lieu de 'idLiability', vous pouvez le mapper ainsi :
        this.liability = {
          ...data,
          idLiability: (data as any).id  // ou simplement data.id si l'interface est modifiée
        };
      },
